...
#define SAMSUNG_CONTROL_CLK        1

#ifdef SAMSUNG_CONTROL_CLK
enum fpc_spi_speed {
	GF_LOW_SPEED        = 0,
	GF_HIGH_SPEED       = 1,
	GF_SUPER_SPEED      = 2,
...
static uint32_t g_spi_speed = GF_LOW_SPEED;
#endif
...
#ifdef SAMSUNG_CONTROL_CLK
static void fpc_spi_clock_set(struct fpc1020_data *data, int speed)
{
	int rc;
...
static int fpc_spi_ioctl_clk_init(struct fpc1020_data *data)
{
	pr_debug("%s: enter\n", __func__);
...
static int fpc_spi_ioctl_clk_enable(struct fpc1020_data *data, uint32_t flag)
{
	int err;
	...
	if (GF_LOW_SPEED == flag) {
		fpc_spi_clock_set(data, SPI_LOW_SPEED);
	} else if (GF_HIGH_SPEED == flag) {
		fpc_spi_clock_set(data, SPI_HIGH_SPEED);
	} else if (GF_SUPER_SPEED == flag) {
		fpc_spi_clock_set(data, SPI_SUPER_SPEED);
	} else {
		pr_err("%s: fail to enable clock, flag(%d) is not in range[0-2]\n", __func__, flag);
	}
...
static int fpc_spi_ioctl_clk_disable(struct fpc1020_data *data)
{
	pr_debug("%s: enter\n", __func__);
...
static int fpc_spi_ioctl_clk_uninit(struct fpc1020_data *data)
{
	pr_debug("%s: enter\n", __func__);

	if (data->clk_enabled)
		fpc_spi_ioctl_clk_disable(data);
...
{
	bool clk_enable = (*buf == '1');
	struct fpc1020_data *fpc1020 = dev_get_drvdata(dev);
	dev_info(dev, "clk_enable: %d\n", clk_enable);
// set spi
#ifdef SAMSUNG_CONTROL_CLK
	if(clk_enable){
		/*if (copy_from_user(&g_spi_speed, (void __user *)arg, sizeof(g_spi_speed))) {
			pr_info("%s GF_IOC_ENABLE_SPI_CLK copy_from_user error \n", __func__);
			retval = -EFAULT;
			break;
		}*/
		fpc_spi_ioctl_clk_enable(fpc1020, g_spi_speed);
	}
	else {
		fpc_spi_ioctl_clk_disable(fpc1020);
	}
#else
...
static int fpc1020_probe(struct platform_device *pdev)
{
	...
	/* Enable spi clock */
	rc = fpc_spi_ioctl_clk_init(fpc1020);
	if(rc)
		goto exit;
	rc = fpc_spi_ioctl_clk_enable(fpc1020, GF_LOW_SPEED);
	if(rc)
		goto exit_probe_clk_enable_failed;
	fpc_spi_ioctl_clk_disable(fpc1020);
#endif
#else
	mutex_init(&fpc1020->lock);
	...
	return rc;
#ifdef SAMSUNG_CONTROL_CLK
exit_probe_clk_enable_failed:
	fpc_spi_ioctl_clk_uninit(fpc1020);
#endif
